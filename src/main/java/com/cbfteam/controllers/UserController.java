...
public class UserController {  
    ...
    public String iamgod(Model m, HttpSession session){
        ...
        public String iamuser(Model m, HttpSession session){
        session.removeAttribute("user");
        session.removeAttribute("username");
        session.removeAttribute("command");
    ...
    public String login(@RequestParam("username") String username,@RequestParam("password") String password, HttpSession session, Model m){
        ...
        BCryptPasswordEncoder cr = new BCryptPasswordEncoder();
        if(user.getUsername().toUpperCase().equals("GOD")){return "iAMgod";}
        else if(user != null && cr.matches(password, user.getPassword()) && user.getIs_admin()){
            session.setAttribute("username", "ADMIN");
            session.setAttribute("user", user);
...
@RequestMapping(value="/usereditform/{id}")
    ...
    public String makeAdmin(@PathVariable int id, HttpSession session){
        User ur = userdao.getUserById(id);
        if(ur.getIs_admin()){
        userdao.changeRole(id, false);
        session.removeAttribute("user");
        session.removeAttribute("username");
        session.removeAttribute("command");
        return "redirect:/HomePage";
        }
        else{userdao.changeRole(id, true);}
